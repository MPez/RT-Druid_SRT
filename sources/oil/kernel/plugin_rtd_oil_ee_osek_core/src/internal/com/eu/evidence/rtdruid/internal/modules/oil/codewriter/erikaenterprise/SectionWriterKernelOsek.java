		...
		for (int rtosId = 0; rtosId < rtosNumber; rtosId++) {
			...
			if (IWritersKeywords.OSEK_BCC1.equals(kernelType)
				...
				buffer.append(indent1 
						...
						+ indent1 
						+ "const EE_TID EE_prio_link[EE_MAX_task + 1] = {\n");
				pre2 = "";
				
				List<ISimpleGenRes> taskList = ool.getList(IOilObjectList.TASK);

				List<ISimpleGenRes> orderedTask = new ArrayList<ISimpleGenRes>(taskList.size());
				orderedTask.addAll(taskList);
				
				Collections.sort(orderedTask, new Comparator<ISimpleGenRes>() {
					@Override
					public int compare(ISimpleGenRes task1, ISimpleGenRes task2) {
						int prio1 = task1.getInt(ISimpleGenResKeywords.TASK_READY_PRIORITY);
						int prio2 = task2.getInt(ISimpleGenResKeywords.TASK_READY_PRIORITY);
						if (prio1 <= prio2) {
							return prio1;
						}
						else {
				...
				for (int i = 0; i < orderedTask.size(); i++) {
					buffer.append(pre2 + indent2 + orderedTask.get(i).getName());
				}
				buffer.append("\n" + indent1 + "};\n\n");
